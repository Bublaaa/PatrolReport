...
const measureTextHeight = (doc, text, width, options = {}) => {
  const { font = "Helvetica", fontSize = 10, padding = 6 } = options;
...
export const generateReportPDF = async (reports, res, imagesByReportId) => {
  ...
  for (let index = 0; index < reports.length; index++) {
    ...
    Y += ROW_HEIGHT;
    const reportTextHeight = measureTextHeight(
      doc,
      report.report,
      USABLE_WIDTH,
      {
    ...
    drawCell(doc, START_X, Y, USABLE_WIDTH, reportTextHeight, report.report, {
      align: "left",
    });
